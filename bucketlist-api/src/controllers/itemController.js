...
exports.criarItem = async (req, res) => {
  try {
    ...
    if (!lista) {
      return res
        .status(403)
        .json({ message: 'Lista não pertence ao usuário ou não existe' });
    }
...
exports.obterItens = async (req, res) => {
  try {
    ...
    const listasUsuario = await Lista.findAll({
      where: whereListas,
      attributes: ['id'],
    });
...
exports.atualizarItem = async (req, res) => {
  try {
    ...
    if (!item || item.lista.usuarioId !== usuarioId) {
      return res
        .status(403)
        .json({ message: 'Item não pertence ao usuário ou não existe' });
    }
    ...
    if (!updated) {
      return res
        .status(404)
        .json({ message: 'Item não encontrado para atualizar' });
    }
