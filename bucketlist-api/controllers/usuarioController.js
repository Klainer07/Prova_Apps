...
exports.atualizarUsuario = async (req, res) => {
    try {
        const { id } = req.params;
        const [atualizados] = await Usuario.update(req.body, { where: { id } });
        if (!atualizados) return res.status(404).json({ message: 'Usuário não encontrado' });
...
exports.deletarUsuario = async (req, res) => {
    try {
        ...
        if (!deletados) return res.status(404).json({ message: 'Usuário não encontrado' });
